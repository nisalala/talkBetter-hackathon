...
export default function ProfilePage() {
  ...
  const [historyFilter, setHistoryFilter] = useState('all') // 'all', 'solo', 'conversation'
  ...
  const handleViewResult = (session) => {
    ...
    // Route to appropriate results page based on session type
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Practice Mode Stats - NEW SECTION */}
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="glass rounded-2xl p-5 border border-indigo-500/30 bg-gradient-to-br from-indigo-500/5 to-purple-500/5">
          <div className="flex items-center gap-3 mb-4">
      ...
      <div className="glass rounded-2xl p-6 mb-6">
        ...
        {filteredHistory.length === 0 ? (
          <div className="text-center py-8">
            ...
            <button
              onClick={() => router.push(historyFilter === 'conversation' ? '/conversation' : '/')}
              className="mt-4 px-4 py-2 rounded-lg bg-indigo-500/20 text-indigo-400 
                       hover:bg-indigo-500/30 transition-colors text-sm"
            >
              {historyFilter === 'conversation' ? 'Start AI Conversation' : 'Start Practicing'}
            </button>
          </div>
        ) : (
